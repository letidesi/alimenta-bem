...
const CreateDonation = () => {
  const [donationData, setDonationData] = useState({ naturalPersonId: "", organizationId: "", itemName: "", amountDonated: 0 });
  ...
  useEffect(() => {
    ...
    const fetchLoggedDonor = async () => {
      try {
        ...
        if (!donorId) {
          setProfileError("Complete seu perfil antes de realizar doações.");
          setLoggedDonorName(donorName);
          return;
        }

        setDonationData((prev) => ({ ...prev, naturalPersonId: donorId }));
        setLoggedDonorName(donorName);
        ...
        if (error?.response?.status === 404) {
          ...
          setProfileError("Não foi possível carregar o perfil do doador logado.");
          setLoggedDonorName("Não identificado");
        }
      }
    };
  ...
  const handleChange = (e) => setDonationData((prev) => ({ ...prev, [e.target.name]: e.target.value }));

  const handleOrganizationChange = async (e) => {
    const selectedOrganizationId = e.target.value;
    setDonationData((prev) => ({ ...prev, organizationId: selectedOrganizationId }));
    if (!selectedOrganizationId) { setOrganizationRequirements([]); return; }

    setRequirementsLoading(true);
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/organization-requirements/${selectedOrganizationId}`);
      setOrganizationRequirements(response.data?.requirements || []);
    } catch {
  ...
  const applyRequirementSuggestion = (requirement) => {
    setDonationData((prev) => ({ ...prev, itemName: requirement.itemName, amountDonated: requirement.quantity }));
  };
    ...
    <div>
      ...
      <form className="create-user-form" onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <select name="organizationId" value={donationData.organizationId} onChange={handleOrganizationChange} required>
            <option value="">Selecione uma instituição</option>
            {institutions.map((org, index) => <option key={index} value={org.id}>{org.name}</option>)}
          </select>
        </div>
        ...
        {!!donationData.organizationId && (
          <div className="form-group requirement-message-box">
            ...
            {requirementsLoading ? (
                ...
                {organizationRequirements.map((item) => (
                  <li key={item.id} className="requirement-list-item">
                    <div>
                      ...
                      <span>Quantidade sugerida: {item.quantity} | Prioridade: {item.type}</span>
                    </div>
                    <button type="button" className="submit-btn-register" onClick={() => applyRequirementSuggestion(item)}>
                      Usar sugestão
                    </button>
                  </li>
                ))}
              ...
              <p className="requirement-message-text">Esta instituição ainda não publicou necessidades específicas.</p>
            )}
          </div>
        )}
            ...
            {donationsHistory.map((donation) => (
              <li key={donation.id} className="donation-history-item">
                <div>
                  ...
                  <span>Instituição: {donation.organizationName} | Quantidade: {donation.amountDonated}</span>
                </div>
                <div className="donation-history-status-box">
                  <span className={getDonationStatusClassName(donation.status)}>{getDonationStatusLabel(donation.status)}</span>
                  {donation.unavailableMessage && <p className="donation-unavailable-message">{donation.unavailableMessage}</p>}
                </div>
              </li>
            ))}
          </ul>
